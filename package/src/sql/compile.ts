...
export function compileProcs(
  ...
  const ctx = { schema, overrides: tableNameOverrides };
...
function columnTs(ctx: CompileContext, table: string, column: string, proc: string): string {
  return columnToTsType(resolveColumn(ctx, table, column, proc));
}
...
function renderModule(
  ...
  const procLines = Object.entries(queries)
    .map(([n, sql]) => `${n}: \`${sql.replace(/\\/g, "\\\\").replace(/`/g, "\\`").replace(/\$\{/g, "\\${").replace(/\n/g, "\\n")}\`,`)
    .join("\n  ");
